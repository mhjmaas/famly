...
export default async function CalendarPage({ params }: PageProps) {
  ...
  return (
    <DashboardLayout dict={dict} lang={lang} title={dict.dashboard.pages.calendar.title}>
      <div className="flex flex-col gap-4">
        <h1 className="text-3xl font-bold text-foreground">
          {dict.dashboard.pages.calendar.title}
        </h1>
        <p className="text-muted-foreground">
          {dict.dashboard.pages.calendar.placeholder}
        </p>
      </div>
    </DashboardLayout>
  );
