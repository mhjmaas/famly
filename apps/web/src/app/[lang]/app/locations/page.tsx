...
export default async function LocationsPage({ params }: PageProps) {
  ...
  return (
    <DashboardLayout dict={dict} lang={lang} title={dict.dashboard.pages.locations.title}>
      <div className="flex flex-col gap-4">
        <h1 className="text-3xl font-bold text-foreground">
          {dict.dashboard.pages.locations.title}
        </h1>
        <p className="text-muted-foreground">
          {dict.dashboard.pages.locations.placeholder}
        </p>
      </div>
    </DashboardLayout>
  );
