...
export default async function AISettingsPage({ params }: PageProps) {
  ...
  return (
    <DashboardLayout dict={dict} lang={lang} title={dict.dashboard.pages.aiSettings.title}>
      <div className="flex flex-col gap-4">
        <h1 className="text-3xl font-bold text-foreground">
          {dict.dashboard.pages.aiSettings.title}
        </h1>
        <p className="text-muted-foreground">
          {dict.dashboard.pages.aiSettings.placeholder}
        </p>
      </div>
    </DashboardLayout>
  );
