...
const bulletOrder: FeatureHighlightBulletKey[] = [...featureHighlightBulletKeys];
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-7xl mx-auto">
          {featureCards.map(({ key, icon: Icon, color, bgColor }) => {
            ...
            return (
              <Card key={key} data-testid="feature-card" className="p-6 hover:shadow-lg transition-all duration-300 hover:-translate-y-1 border-2">
        ...
        <div className="mt-20 max-w-6xl mx-auto">
          <div className="grid md:grid-cols-2 gap-8 items-center">
            <div className="space-y-6">
              <h3 className="text-3xl font-bold text-balance">{dict.highlight.title}</h3>
              <p className="text-lg text-muted-foreground leading-relaxed">{dict.highlight.description}</p>
              <ul className="space-y-3">
                {bulletOrder.map((key) => (
                  <li key={key} className="flex items-center gap-3">
                    <div className="h-2 w-2 rounded-full bg-primary" />
                    <span className="text-muted-foreground">{dict.highlight.bullets[key]}</span>
                  </li>
                ))}
              </ul>
            </div>
            <div className="relative">
              <div className="aspect-square rounded-2xl bg-gradient-to-br from-secondary via-accent to-muted p-8 flex items-center justify-center">
                <div className="text-center space-y-4">
                  ...
                  <p className="text-sm font-medium text-muted-foreground">{dict.highlight.badge}</p>
                </div>
              </div>
            </div>
          </div>
