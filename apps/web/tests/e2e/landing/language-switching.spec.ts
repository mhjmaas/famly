...
test.describe('Landing Page - Language Switching', () => {
  ...
  test('should work when switching language from middle of page', async ({ page }) => {
  ...
  test('should update page metadata when switching language', async ({ page }) => {
    ...
    // Get title in Dutch - should now contain text (may be same or translated)
  ...
  test('should verify language selector button states', async ({ page }) => {
    // Find the language selector group
    const languageGroup = page.locator('[role="group"]').filter({
  ...
  test('should have accessible language selector', async ({ page }) => {
    ...
    const languageGroup = page.locator('[role="group"]').filter({ has: page.getByRole('button').filter({ hasText: /EN|NL/ }) });
    const isVisible = await languageGroup.isVisible();
    expect(isVisible).toBe(true);

    // Get all buttons in the language selector
    const enButton = languageGroup.getByRole('button').filter({ hasText: /EN/ });
    const nlButton = languageGroup.getByRole('button').filter({ hasText: /NL/ });
    ...
    const activeButton = languageGroup.getByRole('button').filter({ hasText: /EN/ });
