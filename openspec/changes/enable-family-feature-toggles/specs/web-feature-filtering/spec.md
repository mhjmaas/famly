...
### Requirement: Dashboard navigation is feature-aware (extends web-dashboard)
The existing dashboard navigation system MUST be enhanced to filter items based on enabled features.

#### Scenario: Navigation hook returns filtered items
- **GIVEN** the existing `useDashboardNavigation` hook
- **WHEN** it is called with the current family's enabled features
- **THEN** it returns only navigation items for enabled features
- **AND** maintains backwards compatibility with existing components
- **AND** preserves the original navigation structure

### Requirement: Dashboard layout loads settings on mount (extends web-dashboard)
The dashboard layout MUST fetch family settings when the user first loads the dashboard.

#### Scenario: Dashboard fetches settings on initial load
- **GIVEN** a user navigating to any dashboard page
- **WHEN** the DashboardLayout component mounts
- **THEN** it dispatches `fetchFamilySettings` for the current family
- **AND** the settings are loaded into Redux
- **AND** localStorage is updated with the latest settings
- **AND** navigation updates to reflect the settings